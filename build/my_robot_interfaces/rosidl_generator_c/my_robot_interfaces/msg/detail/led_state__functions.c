...
my_robot_interfaces__msg__LedState__are_equal(const my_robot_interfaces__msg__LedState * lhs, const my_robot_interfaces__msg__LedState * rhs)
{
  ...
  for (size_t i = 0; i < 3; ++i) {
    if (lhs->led_state[i] != rhs->led_state[i]) {
      return false;
    }
  }
  return true;
  ...
  my_robot_interfaces__msg__LedState * output)
{
  ...
  // led_state
  for (size_t i = 0; i < 3; ++i) {
    output->led_state[i] = input->led_state[i];
  }
  return true;
}
  ...
  my_robot_interfaces__msg__LedState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    my_robot_interfaces__msg__LedState * data =
      (my_robot_interfaces__msg__LedState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_robot_interfaces__msg__LedState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          my_robot_interfaces__msg__LedState__fini(&data[i]);
        }
        free(data);
        return false;
      }
